...
class do_english_format2:
    ...
    def getFromScanned(self):
        ...
        for rect in rects:
            ...
            try:
                ...
                T_nameImg = image[int(house_yc-y-house_H/2 -3):int(house_yc-y+house_H/2 +3), int(house_xc-x+house_W/2)+9:int(house_xc-x+house_W/2)+80]
                TName = pytesseract.image_to_string(T_nameImg, config='--psm 6').strip()
                strp_chars = "|^#;$`-_=*\/‘¢[®°]:"
    ...
    def getFromDigital(self):
        ...
        _ = self.get_digit(d)
        self.get_digit_cen()
        table, preYC = [], self.digit_cen_value[0][0]
        row = []
        for dcv in self.digit_cen_value:
            if dcv[0] == preYC: row.append(dcv[1:3])
            else: 
                preYC = dcv[0]
                table.append(row)
                row = [dcv[1:3]]
        ...
        for tab in table[1]:
            if 'relation' in tab[1].lower(): RelationX = int(tab[0])
            elif 'sex' in tab[1].lower(): SexX = int(tab[0])
            elif 'house' in tab[1].lower(): HouseX = int(tab[0])
            elif 'age' in tab[1].lower(): AgeX = int(tab[0])
        mainTable = table[5:-1]
        for k, tab in enumerate(mainTable):
            if k > 0 and len(tab) == 2:
                results[-1]['name'] = results[-1]['name'] + ' ' + tab[0][1]
                results[-1]['father_name'] = results[-1]['father_name'] + ' ' + tab[1][1]
            elif k > 0 and len(tab) == 1: results[-1]['name'] = results[-1]['name'] + ' ' + tab[0][1]
            else:
                house_no = ''.join([v[1] for v in tab if abs(v[0]-HouseX)<35])
                vote_name = ' '.join([v[1] for v in tab if HouseX+30<v[0]<RelationX-30])
                father_name = ' '.join([v[1] for v in tab if RelationX+30<v[0]<SexX-30])
                vote_id = ' '.join([v[1] for v in tab if AgeX+30<v[0]])
                results.append({'id':vote_id, 'name':vote_name, 'father_name':father_name, 'house_no':house_no, 'PageNumber':self.page_num})
